...
#define KEYBOARD_BUFFER_DIMENSION		32
...
typedef struct console {
	...
	// --- KEYBOARD ---
	u08int	num_lock;		// NUM LOCK key flag.
	u08int	caps_lock;		// CAPS LOCK key flag.
	u08int	scroll_lock;	// SCROLL LOCK key flag.
	u16int	keyb_buf_read;	// The first character to read from the buffer.
	u16int	keyb_buf_write;	// The last character inserted into the buffer.
	u16int	keyb_buf_count;	// How many characters are present into the buffer.
