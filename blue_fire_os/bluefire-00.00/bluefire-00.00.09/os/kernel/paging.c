...
void check_free_frames_integrity() {
	// WARNING: This is a process, so must ends with the auto_kill()
	u32int flag, i;
	u32int *j;

	kprintf("\nChecking %X free frames for integrity...\n\r", (ADDR_TO_PAGE(var_system_memory_amount) - P_ADDR_16MB));
	dbg_pause(3);
	for (i = P_ADDR_16MB; i < ADDR_TO_PAGE(var_system_memory_amount); i++) {
		flag=0;
