...
task_t *create_process(void *routine, s32int argc, s08int **argv, s08int *pname, s32int privilege) {
	...
	new_task->tss.esp0 = ALIGN_DOWN( (u32int)(new_task->pl0_stack) + STACK_SIZE - sizeof(u32int), sizeof(u32int) );
	...
	new_task->tss.eip = (u32int)routine;
 ...
 *	<TODO>
******************************************************************************/
task_t *create_thread(void *routine, s32int argc, s08int **argv, s08int *pname) {
	...
	// Null the stack to enforce the page mapping.
	memset08( (void *)(new_thread->pl0_stack), 0, STACK_SIZE );

	// Setup the privileged stack.
	new_thread->tss.ss0 = KERNEL_STACK;
	new_thread->tss.esp0 = ALIGN_DOWN( (u32int)(new_thread->pl0_stack) + STACK_SIZE - sizeof(u32int), sizeof(u32int) );
	...
	new_thread->flags.type = KERNEL_THREAD_TYPE;

	// Get the father's privilege.
	privilege = new_thread->privilege = current_task->privilege;
	...
	// same address space.
	proc_father = current_task;
	while( proc_father->flags.type == KERNEL_THREAD_TYPE ) {
	...
	new_thread->pdbr = current_task->pdbr;

	// Get the same pdbr counter of the father.
	new_thread->pdbr_update_counter = current_task->pdbr_update_counter;

	// Create the thread stack.
	new_thread->tss.ss = (privilege == KERNEL_PRIVILEGE) ? KERNEL_STACK : USER_STACK | 3;
	if ( privilege == KERNEL_PRIVILEGE ) {
		new_thread->tss.esp = (size_t) task_setup_stack( argc, argv, new_thread->tss.esp0 + sizeof(u32int) );
	} else {
		//TODO
		...
		if( new_thread->stack == NULL ) {
			kprintf("%s: not enough space in the heap! Cannot create thread '%s'\n.", __FUNCTION__, pname);
			kfree( new_thread );
			kfree( (void *)(new_thread->pl0_stack) );
	...
	memset32( new_thread->tss.io_map, 0xffffffff, IO_MAP_SIZE );
	...
	// Setup starting address (program counter).
	new_thread->tss.cs = (privilege == KERNEL_PRIVILEGE) ? KERNEL_CODE : (USER_CODE | 3);
	new_thread->tss.eip = (u32int)routine;

	// Store the process name.
	// The last character must be ever '\0' (end of string).
	strncpy08( new_thread->name, pname, sizeof(new_thread->name) - 2 );

	// Set the current working directory.
	new_thread->cwd = current_task->cwd;

	// Set the process credentials.
	new_thread->pid = new_pid();
	new_thread->uid = current_task->uid;
	new_thread->euid = current_task->euid;
	new_thread->suid = current_task->suid;
	new_thread->fsuid = current_task->fsuid;
	new_thread->gid = current_task->gid;
	new_thread->egid = current_task->egid;
	new_thread->sgid = current_task->sgid;
	new_thread->fsgid = current_task->fsgid;

	// Get the father.
	new_thread->father = current_task;

	// Set the console.
	new_thread->console = current_task->console;
